    ...
    describe("argument:", () => {
        ...
        describe("buffer:", () => {
            ...
            it("throws if buffer argument is not buffer", () => {
                expect(() =>
                    readBigInt64LE(ZERO_64BIT_SIGNED_INTEGER as any)
                ).toThrow(/argument must be of type/);
                expect(() => readBigInt64LE("buffer" as any)).toThrow(
                    /argument must be of type/
                );
                expect(() => readBigInt64LE(0 as any)).toThrow(
                    /argument must be of type/
                );
                expect(() => readBigInt64LE({ [0]: "hello" } as any)).toThrow(
                    /argument must be of type/
                );
            });
        });
        ...
        describe("offset:", () => {
            ...
            it("throws 'argument type' error if offset is not a number", () => {
                expect(() =>
                    readBigInt64LE(buffer, BigInt("1") as any)
                ).toThrow(/argument must be of type/);
                expect(() => readBigInt64LE(buffer, "1" as any)).toThrow(
                    /argument must be of type/
                );
            });
        });
